...
import { useGsapAnimations } from "@/hooks/useScrollReveal";
...
export default function LandingPage() {
  ...
  const gsapRef = useGsapAnimations<HTMLDivElement>();
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-background" ref={gsapRef}>
      ...
      <section className="flex flex-col items-center text-center px-5 sm:px-8 pt-16 sm:pt-24 pb-12 sm:pb-16 gap-8">
        <div data-gsap="hero" className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 text-sm font-medium text-primary">
        ...
        <div data-gsap="hero" className="flex flex-col gap-4 max-w-3xl">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-foreground leading-[1.1] tracking-tight">
            ...
            <span
              ...
              style={{
                background: "linear-gradient(110deg, #008060 30%, #00b386 50%, #008060 70%)",
        ...
        <div data-gsap="hero" className="flex flex-col sm:flex-row gap-3 mt-2">
          <Link
            href={appHref}
            data-gsap="pulse"
            className="rounded-lg bg-primary px-8 py-3.5 text-base font-semibold text-primary-foreground hover:bg-primary/90 transition-all duration-200 shadow-sm hover:-translate-y-0.5"
          >
            Get Started — It&apos;s Free
          </Link>
          <a
            href="#how-it-works"
            className="rounded-lg border border-border px-8 py-3.5 text-base font-medium text-foreground hover:bg-muted transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
          >
            How It Works
          </a>
        </div>
      ...
      {/* Preview Cards */}
      <section className="px-5 sm:px-8 py-8 sm:py-12" data-gsap-section="cards">
        <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4">
          {/* Card 1: Real-time Camera AI */}
          <div data-gsap="card" className="group relative aspect-[9/16] sm:aspect-[3/4] rounded-xl border border-border bg-gradient-to-b from-primary/10 to-primary/5 flex flex-col items-center justify-between p-5 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-primary/30 hover:-translate-y-2">
          ...
          <div data-gsap="card" className="group relative aspect-[9/16] sm:aspect-[3/4] rounded-xl border border-border bg-gradient-to-b from-[#008060]/8 to-[#008060]/3 flex flex-col items-center justify-between p-5 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-primary/30 hover:-translate-y-2">
            <div className="flex-1 flex items-center justify-center w-full">
          ...
          {/* Card 3: Voice-first */}
          <div data-gsap="card" className="group relative aspect-[9/16] sm:aspect-[3/4] rounded-xl border border-border bg-gradient-to-b from-primary/8 to-transparent flex flex-col items-center justify-between p-5 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-primary/30 hover:-translate-y-2">
            <div className="flex-1 flex items-center justify-center w-full">
      ...
      {/* Features */}
      <section className="px-5 sm:px-8 py-16 sm:py-20" id="features" data-gsap-section="features">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 data-gsap="heading" className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight">
              Four modes, one assistant
            </h2>
            <p data-gsap="subtitle" className="text-muted-foreground mt-3 text-lg">Each mode is optimized for a specific scenario</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            {FEATURES.map((f) => (
              <button
                ...
                className="group rounded-xl border border-border bg-card p-6 flex flex-col gap-4 text-left transition-all duration-300 hover:border-primary/30 hover:shadow-xl hover:-translate-y-1 cursor-pointer"
                aria-label={`${f.title}. Tap to hear description.`}
              >
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10 text-primary transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 group-hover:bg-primary/20">
                    {f.icon}
                  </div>
      ...
      {/* How it works */}
      <section className="px-5 sm:px-8 py-16 sm:py-20 bg-muted/50" id="how-it-works" data-gsap-section="steps">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            <h2 data-gsap="heading" className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight">
              How it works
            </h2>
            <p data-gsap="subtitle" className="text-muted-foreground mt-3 text-lg">Get started in under a minute</p>
          </div>
          <div className="flex flex-col gap-8">
            {HOW_IT_WORKS.map((item) => (
              <button
                ...
                className="flex gap-5 items-start text-left cursor-pointer rounded-xl p-3 -m-3 transition-all duration-300 hover:bg-card hover:shadow-sm"
                aria-label={`Step ${item.step}: ${item.title}. Tap to hear.`}
              >
                <div data-gsap="step-num" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground font-bold text-sm transition-transform duration-300 hover:scale-110">
                  {item.step}
                </div>
                <div>
      ...
      {/* Tech Stack */}
      <section className="px-5 sm:px-8 py-16 sm:py-20" data-gsap-section="tech">
        <div className="max-w-3xl mx-auto text-center">
          <h2 data-gsap="heading" className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight mb-3">
            Built with Google Cloud
          </h2>
          <p data-gsap="subtitle" className="text-muted-foreground text-lg mb-8">Enterprise-grade infrastructure, open-source stack</p>
          <div className="flex flex-wrap justify-center gap-2.5">
            {TECH_STACK.map((tech) => (
              <span
                key={tech}
                data-gsap="pill"
                className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium text-foreground shadow-sm transition-all duration-200 hover:border-primary/30 hover:shadow-md hover:-translate-y-0.5"
              >
                {tech}
              </span>
            ))}
      ...
      {/* CTA */}
      <section className="px-5 sm:px-8 py-16 sm:py-20 bg-primary/5" data-gsap-section="cta">
        <div className="max-w-lg mx-auto text-center flex flex-col items-center gap-5">
          <h2 data-gsap="cta" className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight">
            Ready to try SightLine?
          </h2>
          <p data-gsap="cta" className="text-muted-foreground text-lg max-w-sm">
            Free to use. No credit card required. Start seeing the world through AI today.
          </p>
          <div data-gsap="cta" className="flex flex-col sm:flex-row gap-3">
            <Link
              href={appHref}
              data-gsap="pulse"
              className="rounded-lg bg-primary px-8 py-3.5 text-base font-semibold text-primary-foreground hover:bg-primary/90 transition-all duration-200 shadow-sm hover:-translate-y-0.5"
      ...
      {/* Footer */}
      <footer data-gsap="footer" className="px-5 sm:px-8 py-8 border-t border-border">
